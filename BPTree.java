...
public class BPTree {
    ...
    private Node helperSearch(Node nodeBook, String title) {
        if(nodeBook.is_Leaf == false) {
            ...
            if(title.compareTo(titleComparison) < 0) {
                return helperSearch(position.child_ptr.get(0), title);
            }

            titleComparison = position.titleK.get(size1);
            if(title.compareTo(titleComparison) >= 0) {
                return helperSearch(position.child_ptr.get(size2), title);
            }
                ...
                for(int i=0; i<size3; i++) {
                    ...
                    if(title.compareTo(titleComparison) >= 0) {
                        ...
                        if (title.compareTo(titleComparison) < 0) {
                            return helperSearch(position.child_ptr.get(i + 1), title);
                        }
                    }
                }
            }
    ...
    public void search(String text) {
        if (text != null)
        {
            if (rootNode != null)
            {
                LeafNode correctL = (LeafNode) helperSearch(rootNode, text);
